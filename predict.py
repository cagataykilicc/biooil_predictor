...
def load_artifacts(models_dir: Path):
    ...
    if not models_dir.exists():
        sys.exit(
            ...
            f"    .venv\\Scripts\\python.exe train_model.py --data merged_dataset.csv"
        )
    models = {}
    try:
...
def predict_ensemble(models, features_array, feature_names):
    """11 modelle tahmin yapar; ortalama, std, min, max döner.

    Modeller feature_names ile fit edildiğinden, tahmin için
    ...
    preds_list = []
    for name in MODEL_KEYS:
        preds_list.append(models[name].predict(X_df))

    preds = np.array(preds_list)  # shape: (11, n_samples)
    res = {
...
FEEDSTOCK_PRESETS = {
    "1": {
        "name": "Pirinc sapi (Rice straw)",
        "Cellulose_pct": 36.5,
        "Hemicellulose_pct": 24.0,
        "Lignin_pct": 15.4,
    },
    "2": {
        "name": "Bugday sapi (Wheat straw)",
        "Cellulose_pct": 38.6,
        "Hemicellulose_pct": 28.5,
        "Lignin_pct": 15.0,
    },
    "3": {
        "name": "Misir kocani (Corn cob)",
        "Cellulose_pct": 35.0,
        "Hemicellulose_pct": 28.0,
        "Lignin_pct": 15.0,
    },
    "4": {
        "name": "Findik kabugu (Hazelnut shell)",
        "Cellulose_pct": 26.8,
        "Hemicellulose_pct": 30.4,
        "Lignin_pct": 42.9,
    },
    "5": {
        "name": "Pamuk sapi (Cotton stalk)",
        "Cellulose_pct": 45.0,
        "Hemicellulose_pct": 21.0,
        "Lignin_pct": 30.0,
    },
    "6": {
        "name": "Ayciçek sapi (Sunflower stalk)",
        "Cellulose_pct": 42.1,
        "Hemicellulose_pct": 29.7,
        "Lignin_pct": 13.4,
    },
}
...
def save_prediction_to_history(sample_name, values, res, opt_temp, opt_yield):
    ...
    # Bireysel tahminleri ekle
    for name in MODEL_KEYS:
        col_name = f"{name.replace('_', ' ').title().replace(' ', '')}_Pred"
        new_row[col_name] = float(res[name][0])
...
def interactive_mode(models, metadata):
    print("=" * 70)
    print("  TARIMSAL BİYOKÜTLE PİROLİZİ — BİO-YAĞ VERİMİ TAHMİN PROGRAMI (11 MODEL)")
    print("=" * 70)
    ...
    try:
        ...
        for key, val in FEEDSTOCK_PRESETS.items():
            print(
                f"  [{key}] {val['name']} (Cel: {val['Cellulose_pct']}%, Hem: {val['Hemicellulose_pct']}%, Lig: {val['Lignin_pct']}%)"
            )
        ...
        for name in MODEL_KEYS:
            ...
            print(f"    {nice_name:<20}: {res[name][0]:.2f}%")
        ...
        if abs(opt_temp - pt) <= 5:
            ...
            print(
                f"    >> Tavsiye: Sıcaklığı {pt}°C değerinden {opt_temp}°C değerine ayarlamak,"
            )
        ...
        # En iyi modelin performans bilgisi (catboost'u örnek alalım)
        cb = metadata["models"]["catboost"]
        print(f"\n  Model performansı (CatBoost, test seti üzerinde):")
        print(
            f"    R2 = {cb['r2_test']:.3f}, RMSE = {cb['rmse_test']:.2f}, "
...
def main():
    ...
    parser.add_argument("--input", "-i", help="Toplu mod için girdi CSV dosyası")
    parser.add_argument(
        "--output",
        "-o",
